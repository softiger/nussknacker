...
class KafkaClient(kafkaAddress: String, zkAddress: String) {
  ...
  private val zkClient = KafkaZkClient(zkAddress, false, 10000, 10000, 100, Time.SYSTEM, "", "")
  ...
  def createTopic(name: String, partitions: Int = 5) = {
    ...
    adminClient.createOrUpdateTopicPartitionAssignmentPathInZK(name, replicaAssignment, new Properties, update = true)
  }
