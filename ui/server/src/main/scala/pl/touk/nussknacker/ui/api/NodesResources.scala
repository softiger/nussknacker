...
class NodesResources(val processRepository: FetchingProcessRepository[Future], typeToConfig: Map[ProcessingType, ProcessingTypeData])(implicit val ec: ExecutionContext)
  extends ProcessDirectives with FailFastCirceSupport {

  private val providers: Map[ProcessingType, Option[NodeData => Future[Option[NodeAdditionalInfo]]]] = typeToConfig.mapValuesNow(pt => ScalaServiceLoader
        ...
        entity(as[NodeData]) { nodeData =>
          complete {
            processRepository.fetchLatestProcessDetailsForProcessId[CanonicalProcess](processId.id).map[ToResponseMarshallable] {
              case Some(process) =>
                providers.get(process.processingType).flatten match {
